...
Do it again with Gallery
- Test if http://localhost:3000/gallery works (no page, redirect)
- Test if http://localhost:3000/gallery?page=4 works (starting not on page 1)
- Test if http://localhost:3000/gallery?art=ordell+another+guitar+piece+idk works (start on art piece)
    - current problem: doubled up if on the same page
- Test if http://localhost:3000/gallery?include=irl&exclude=human works (tags are implemented)
- http://localhost:3000/gallery?include=irl&exclude=human&page=2&art=nelly 
- http://localhost:3000/gallery?page=4&include=oc should redirect back to page 3